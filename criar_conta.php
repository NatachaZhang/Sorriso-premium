      ...
      <div class="bg-white rounded-2xl shadow p-6 md:p-8">
        ...
        <?php if (!empty($signupError)): ?>
          <div class="mb-4 rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-900 text-sm">
            <?php echo h($signupError); ?>
          </div>
        <?php endif; ?>

        <form id="signupForm" action="auth/register.php" method="post" class="space-y-4">
          <input type="hidden" name="csrf_token" value="<?php echo h($_SESSION['csrf_token']); ?>">

          <div class="grid md:grid-cols-2 gap-4">
            <div>
              <label for="f-nome" class="block text-sm font-medium text-slate-700 mb-1">Nome</label>
              <input id="f-nome" name="fNome" type="text" required minlength="2"
                     value="<?php echo h((string)($old['fNome'] ?? '')); ?>"
            ...
            <div>
              ...
              <input id="f-apelido" name="fApelido" type="text" required minlength="2"
                     value="<?php echo h((string)($old['fApelido'] ?? '')); ?>"
                     class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" />
            </div>
          </div>

          <div>
            <label for="f-data-nascimento" class="block text-sm font-medium text-slate-700 mb-1">Data de Nascimento</label>
            <input id="f-data-nascimento" name="fDataNascimento" type="date" required
                   value="<?php echo h((string)($old['fDataNascimento'] ?? '')); ?>"
                   class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" />
          </div>

          <div>
            <label for="f-telemovel" class="block text-sm font-medium text-slate-700 mb-1">Telemóvel</label>
            <input id="f-telemovel" name="fTelemovel" type="tel" required
                   inputmode="numeric" autocomplete="tel"
                   placeholder="Ex: 912345678 ou +351912345678"
                   value="<?php echo h((string)($old['fTelemovel'] ?? '')); ?>"
                   class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" />
          </div>

          <div>
            <label for="f-email" class="block text-sm font-medium text-slate-700 mb-1">Endereço de e-mail</label>
            <input id="f-email" name="fEmail" type="email" required
                   value="<?php echo h((string)($old['fEmail'] ?? '')); ?>"
                   class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" />
          </div>

          <div>
            <label for="f-password" class="block text-sm font-medium text-slate-700 mb-1">Palavra-passe</label>
            <input id="f-password" name="fPassword" type="password" required minlength="8"
                   class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" />
          </div>

          <div>
            <label for="f-confirm-password" class="block text-sm font-medium text-slate-700 mb-1">Confirmar palavra-passe</label>
            <input id="f-confirm-password" name="fConfirmPassword" type="password" required minlength="8"
                   class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500" />
          </div>

          <button type="submit" class="w-full rounded-full bg-sky-500 py-2 text-sm font-semibold text-white hover:bg-sky-400">
            Criar uma conta
          </button>

          <p class="text-center text-xs md:text-sm text-slate-600">
            Já tens conta?
            <a href="login.php" class="text-sky-600 hover:underline">Fazer login</a>
          </p>
        </form>
      </div>
    </div>
